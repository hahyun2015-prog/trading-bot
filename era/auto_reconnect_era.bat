...
rem (2026-08-13) 사용자가 세운 정지는 절대 덮어쓰지 않는다.
rem   2026-08-12 사고: 이 스크립트가 끝에서 system_stopped.flag 를 무조건 지워
rem   사용자의 정지 의사가 지워지고 실주문이 나갔다. 먼저 확인하고 물러난다.
if exist "%~dp0..\system_stopped.flag" (
    echo [STOP] system_stopped.flag 가 있습니다. 재기동하지 않고 종료합니다.
    exit /b 0
)
if exist "%~dp0..\emergency_kill.flag" (
    echo [STOP] emergency_kill.flag 가 있습니다. 재기동하지 않고 종료합니다.
    exit /b 0
)

rem 자체 억제 마커 - 60초 대기 중 워치독이 겹쳐 도는 것을 막는다.
rem   사용자 정지 플래그와 물리적으로 다른 파일이라 서로를 지우지 않는다.
