...
# ── AMATS 추가 (2026-08-10) ────────────────────────────────────────────────
# 원래 auth()/auth_ws()는 발급 실패 시 고정 문구만 출력하고 changeTREnv() 없이
# 조기 반환했다. 그러면 _TRENV가 갱신되지 않은 채 남아 호출부가 getTREnv().my_url_ws
# 에서 AttributeError로 터지고, 서버가 준 진짜 사유는 어디에도 남지 않는다.
# 2026-08-08~08-10 야간수집기가 이틀 반 죽어 있었는데 로그만으로는 원인을 알 수 없어,
# requests.post를 직접 계측하고서야 EGW00103(유효하지 않은 AppKey)임을 확인했다.
#
# 대응이 코드마다 정반대라 구분이 반드시 필요하다.
#   EGW00103 유효하지 않은 AppKey  → 재발급 전까지 재시도는 의미 없음
#   EGW00133 1분당 1회 발급 제한   → 잠시 뒤 재시도하면 됨
class KisAuthError(Exception):
    """발급 실패. code에 KIS 오류코드(EGW00103 등)를 담는다."""

    def __init__(self, stage, status, code, message):
        self.stage, self.status, self.code, self.message = stage, status, code, message
        super().__init__(f"{stage} 실패 (HTTP {status} {code}): {message}")

    @property
    def is_rate_limited(self):
        return self.code == "EGW00133"

    @property
    def is_invalid_key(self):
        return self.code == "EGW00103"


def _describe_failure(res):
    """KIS 응답에서 (오류코드, 메시지)를 뽑는다. 형식이 엔드포인트마다 다르다."""
    try:
        j = res.json()
    except Exception:
        return "", res.text[:200]
    code = j.get("error_code") or j.get("msg_cd") or ""
    msg = j.get("error_description") or j.get("msg1") or str(j)[:200]
    return code, msg
# ──────────────────────────────────────────────────────────────────────────
...
def auth(svr="prod", product=_cfg["my_prod"], url=None):
    ...
    if saved_token is None:  # 기존 발급 토큰 확인이 안되면 발급처리
        ...
        if rescode == 200:  # 토큰 정상 발급
            ...
            _code, _msg = _describe_failure(res)
            print(f"[KIS 인증] 접근토큰 발급 실패 — HTTP {rescode} {_code} {_msg}")
            raise KisAuthError("접근토큰 발급", rescode, _code, _msg)
    else:
...
def auth_ws(svr="prod", product=_cfg["my_prod"]):
    ...
    if rescode == 200:  # 토큰 정상 발급
        ...
        _code, _msg = _describe_failure(res)
        print(f"[KIS 인증] 웹소켓 승인키 발급 실패 — HTTP {rescode} {_code} {_msg}")
        raise KisAuthError("승인키 발급", rescode, _code, _msg)
