...
def main():
    ...
    MARGIN_CAP_LIST = [0.20, 0.30, 0.40, 0.50]
    SL_PT_LIST = [2.0, 2.5, 3.0, 3.5]
    TP_PT_LIST = [4.0, 5.0, 6.0, 7.0]
    ATR_CUTOFF_LIST = [None, 0.6, 0.8]

    # {margin_cap: [results]}
    cap_results = {mc: [] for mc in MARGIN_CAP_LIST}
    ...
    for K in K_LIST:
        ...
        sk_equity = backtest_isf_cached(df_sk, K=0.18, SL_PCT=1.2, TP_PCT=2.0, TS_ENABLED=True, TS_ACTIVATE_PCT=1.5, TS_TRAIL_PCT=0.7, LONG_THRESH=0.03)
            ...
            for sl in SL_PT_LIST:
                for tp in TP_PT_LIST:
                    for cutoff in ATR_CUTOFF_LIST:
                        ...
                        cap_results[m_cap].append({
                            'K': K, 'SL': sl, 'TP': tp, 'ATR_CUTOFF': cutoff,
                            'cagr': cagr, 'mdd': mdd, 'final_cap': final_cap
                        })

    print("\n" + "="*85)
    print("                증거금 캡(MARGIN_CAP)별 최적 전략 성과 비교")
    print("="*85)
    for mc in MARGIN_CAP_LIST:
        lst = cap_results[mc]
        lst.sort(key=lambda x: x['cagr'], reverse=True)
        print(f"\n[선물 증거금 배정 한도: {mc*100:.0f}%]")
        print(f" {'순위':<3} │ {'K값':<4} │ {'손절pt':<5} │ {'익절pt':<5} │ {'ATR필터':<6} │ {'CAGR (%)':>10} │ {'MDD (%)':>10}")
        print("─"*85)
        for idx, r in enumerate(lst[:3]):
            rank = idx + 1
            cutoff_str = f"{r['ATR_CUTOFF']:.1f}pt" if r['ATR_CUTOFF'] is not None else "None"
            print(f" {rank:<3} │ {r['K']:<4.2f} │ {r['SL']:<5.1f} │ {r['TP']:<5.1f} │ {cutoff_str:<6} │ {r['cagr']:>10.2f}% │ {r['mdd']:>10.2f}%")
    print("="*85)
