...
def run_sar_or_bb_replica(df, strategy, Q=0.00005, R=1.0, mult=0.6, atr_cutoff=0.5,
                           ...
                           # [2026-08-14 청산연구 v2] 청산 오버레이 — 전부 None/False가 기본이며
                           # 그 경우 아래 블록은 통째로 비활성이고 종전 동작과 100% 동일하다(골든 대조).
                           #   ovl_dirs           'both'|'long'|'short'  오버레이 적용 방향
                           #   ovl_tp_r           손절폭(sl_limit) 대비 R배수 익절 (지정가 체결)
                           #   ovl_tp_atr         ATR14 배수 익절 (지정가 체결)
                           #   ovl_be_trigger_atr MFE가 이 ATR배수 넘으면 손절선을 진입가로 (역지정 체결)
                           #   ovl_be_offset_atr  본전선 오프셋(ATR배수, 기본 0)
                           #   ovl_giveback_frac  최대이익 대비 이 비율 반납 시 청산 (역지정)
                           #   ovl_giveback_min_atr 되돌림 청산 활성화 최소 MFE(ATR배수)
                           #   ovl_time_bars      진입 후 N봉 경과 시 종가 청산
                           #   ovl_chand_mult     샹들리에 트레일 = 피크 ∓ mult*ATR14 (역지정)
                           #   ovl_sar_off        SAR 역전 청산 비활성(대안 트레일 단독 비교용)
                           #   ovl_trail_delay_atr MFE가 이 ATR배수 넘기 전엔 SAR 역전 청산 비활성
                           #   ovl_opp_signal     반대방향 돌파 목표가 이탈 시 청산
                           #   ovl_partial_frac/_tp_r  목표 도달 시 비중 일부 분할청산(다계약 전제)
                           ovl_dirs='both', ovl_tp_r=None, ovl_tp_atr=None,
                           ovl_be_trigger_atr=None, ovl_be_offset_atr=0.0,
                           ovl_giveback_frac=None, ovl_giveback_min_atr=0.0,
                           ovl_time_bars=None, ovl_chand_mult=None, ovl_sar_off=False,
                           ovl_trail_delay_atr=None, ovl_opp_signal=False,
                           ovl_partial_frac=None, ovl_partial_tp_r=None,
                           # [2026-08-15 SAR제거 검증] ATR 손절 자체를 끄는 스위치.
                           # False(기본)이면 _sl_limit()가 종전 값을 그대로 돌려주므로 동작 100% 동일.
                           ovl_sl_off=False,
                           block_entry_times=None,
                           return_trades=False):
    n = len(df)
    if n < kf_window + 10:
        return None
    ...
    def _sl_limit(mult_):
        # [2026-08-15] ovl_sl_off=True면 손절이 물리적으로 발동 불가능한 값을 돌려준다.
        # 기본 False에서는 아래 종전 경로를 그대로 탄다.
        if ovl_sl_off:
            return 1e9
    ...
    # [2026-08-14 청산연구 v2] SAR 역전 청산 게이트. 기본값(ovl_sar_off=False,
    # ovl_trail_delay_atr=None)에서 항상 True를 돌려주므로 종전 동작과 동일하다.
    def _sar_gate(pos_, peak_, entry_, atr_):
        if not _OVL_ON:
            return True
        if ovl_dirs == 'long' and pos_ != 1:
            return True
        if ovl_dirs == 'short' and pos_ != -1:
            return True
        if ovl_sar_off:
            return False
        if ovl_trail_delay_atr is not None:
            mfe_ = (peak_ - entry_) if pos_ == 1 else (entry_ - peak_)
            if mfe_ < ovl_trail_delay_atr * atr_:
                return False
        return True
    ...
    # [2026-08-14 청산연구 v2] 오버레이 상태. 기본 경로에선 값이 바뀌지 않는다.
    ovl_rem, ovl_booked_pt, ovl_reason = 1.0, 0.0, None
    _OVL_ON = any(v is not None for v in (ovl_tp_r, ovl_tp_atr, ovl_be_trigger_atr,
                                          ovl_giveback_frac, ovl_time_bars, ovl_chand_mult,
                                          ovl_trail_delay_atr, ovl_partial_frac)) \
              or bool(ovl_sar_off) or bool(ovl_opp_signal) or bool(ovl_sl_off)
    entry_time = None
    ...
    for i in range(n):
        ...
        if pos != 0:
                ...
                if strategy == 'sar':
                    if sar.bull:
                        ...
                        if sar_tick_mode:
                            ...
                            if force_close or vol_force_close:
                                exit_price, is_force = c_close, True
                            elif (_sar_gate(pos, peak_price, entry_price, atr14)
                                  and c_low <= sar.sar):
            ...
            # ── [2026-08-14 청산연구 v2] 청산 오버레이 ──────────────────────────
            # 우선순위: (a) 기존 로직이 이미 청산을 냈으면 그대로 둔다(보수적),
            # (b) 역지정 성격(본전·샹들리에·되돌림·반대신호) → 종가 캡 체결,
            # (c) 지정가 성격(익절 목표) → 목표가 체결, (d) 시간청산 → 종가.
            # 봉 내 순서를 알 수 없으므로 역지정을 지정가보다 먼저 본다(불리한 쪽 우선).
            ovl_reason = None
            if _OVL_ON and exit_price is None and strategy == 'sar' and not (
                    (ovl_dirs == 'long' and pos != 1) or (ovl_dirs == 'short' and pos != -1)):
                _mfe = (peak_price - entry_price) if pos == 1 else (entry_price - peak_price)
                _lv, _rs = None, None
                # (b) 역지정 계열 — 가장 불리한(먼저 닿는) 레벨을 고른다
                _cands = []
                if ovl_be_trigger_atr is not None and _mfe >= ovl_be_trigger_atr * atr14:
                    _cands.append((entry_price + (ovl_be_offset_atr * atr14) * (1 if pos == 1 else -1), '본전이동'))
                if ovl_chand_mult is not None:
                    _cands.append((peak_price - ovl_chand_mult * atr14 * (1 if pos == 1 else -1), '샹들리에'))
                if ovl_giveback_frac is not None and _mfe > 0 and _mfe >= ovl_giveback_min_atr * atr14:
                    _cands.append((entry_price + (_mfe * (1.0 - ovl_giveback_frac)) * (1 if pos == 1 else -1), 'MFE되돌림'))
                if ovl_opp_signal:
                    _t = target_short if pos == 1 else target_long
                    if np.isfinite(_t):
                        _cands.append((_t, '반대신호'))
                for _c, _r in _cands:
                    if pos == 1 and c_low <= _c and (_lv is None or _c > _lv):
                        _lv, _rs = _c, _r
                    elif pos == -1 and c_high >= _c and (_lv is None or _c < _lv):
                        _lv, _rs = _c, _r
                if _lv is not None:
                    exit_price = min(c_close, max(_lv, c_low)) if pos == 1 else max(c_close, min(_lv, c_high))
                    ovl_reason = _rs
                # (c) 지정가 익절
                if exit_price is None:
                    _tp = None
                    if ovl_tp_r is not None:
                        _tp = ovl_tp_r * sl_limit
                    if ovl_tp_atr is not None:
                        _v = ovl_tp_atr * atr14
                        _tp = _v if _tp is None else min(_tp, _v)
                    if _tp is not None:
                        _px = entry_price + _tp * (1 if pos == 1 else -1)
                        if (pos == 1 and c_high >= _px) or (pos == -1 and c_low <= _px):
                            exit_price, ovl_reason = _px, 'R익절'
                # (d) 시간 청산
                if exit_price is None and ovl_time_bars is not None and (i - entry_idx) >= ovl_time_bars:
                    exit_price, ovl_reason = c_close, '시간청산'
                # (e) 분할 청산 — 목표 도달 시 비중 일부만 실현하고 잔량은 계속 보유
                if exit_price is None and ovl_partial_frac is not None and ovl_rem >= 1.0 \
                        and ovl_partial_tp_r is not None:
                    _px = entry_price + (ovl_partial_tp_r * sl_limit) * (1 if pos == 1 else -1)
                    if (pos == 1 and c_high >= _px) or (pos == -1 and c_low <= _px):
                        _raw = (_px - entry_price) if pos == 1 else (entry_price - _px)
                        ovl_booked_pt += ovl_partial_frac * (_raw - SLIP_EXIT_NORMAL)
                        cap -= entry_price * point_value * commission_rate * ovl_partial_frac
                        ovl_rem = 1.0 - ovl_partial_frac
            ...
            if exit_price is not None:
                ...
                if ovl_reason is not None:
                    exit_reason = ovl_reason
                elif is_force:
                    exit_reason = '강제청산'
                elif strategy == 'sar':
                ...
                # [2026-08-14 청산연구 v2] ovl_rem=1.0, ovl_booked_pt=0.0(기본)이면 종전과 동일.
                if ovl_rem != 1.0 or ovl_booked_pt != 0.0:
                    raw_pnl = ovl_rem * raw_pnl + ovl_booked_pt / 1.0
                    exit_slip = exit_slip * ovl_rem
                    _net_units = ovl_rem + (1.0 - ovl_rem)
                else:
                    _net_units = 1.0
        ...
        if c_high >= target_long:
            ...
            # closes[i-1]을 c_close로.
                ...
                # [2026-08-16 경로효과 실험] 지정된 시각의 진입을 건너뛴다.
                # None(기본)이면 이 블록은 실행되지 않아 기존 동작과 100% 동일.
                if block_entry_times is not None and dt_index[i] in block_entry_times:
                    continue
                ...
                ovl_rem, ovl_booked_pt, ovl_reason = 1.0, 0.0, None   # [2026-08-14 v2]
            ...
            # [2026-08-11 미래참조 제거] LONG 쪽과 동일한 이유(위 주석 참조).
                ...
                _dir = 1 if reverse_entry else -1
                # [2026-08-16 경로효과 실험] 위 LONG 쪽과 같은 이유.
                if block_entry_times is not None and dt_index[i] in block_entry_times:
                    continue
