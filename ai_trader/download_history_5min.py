...
class DataDownloader5Min:
    def __init__(self):
        ...
        self.max_requests_per_stock = 8 # 8번 = 약 7200개 캔들 (5분봉 기준 약 4.5개월)
        
        print("[DataDownloader5Min] 키움증권 서버 로그인 중...")
    ...
    def _on_login(self, err_code):
        if err_code == 0:
            print("[DataDownloader5Min] 로그인 성공")
        else:
            print(f"[DataDownloader5Min] 로그인 실패 (에러코드: {err_code})")
        if self.login_loop:
    ...
    def download_data(self):
        ...
        for stock in self.target_stocks:
            ...
            print(f"\n[다운로드 시작] {stock['name']} ({self.current_stock_code}) - 과거 5분봉 수집")
            
            # 이전 백테스트 데이터 삭제 (5분봉으로 교체하기 위해 테이블 클리어)
    ...
    def _request_tr(self, next_flag):
        ...
        self.kiwoom.dynamicCall("SetInputValue(QString, QString)", "틱범위", "5") # 5분봉 설정 (핵심 변경사항)
