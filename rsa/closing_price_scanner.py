...
class ClosingPriceScanner:
    ...
    def check_market_regime(self):
        ...
        try:
            ...
            res_day = requests.get(iframe_url, headers=self.headers, timeout=5)
    ...
    def scan_closing_price_stocks(self):
        ...
        for code, name in candidates:
            ...
            # 2. 접 (좁은 이격 및 윗꼬리)
            # 윗꼬리가 전체 몸통의 10% 미만일 것
            body = close - open_p
            upper_shadow = high - close
            is_clean_candle = body > 0 and (upper_shadow / body) < 0.10
            ...
            # 3. 거 (거래량 폭발)
            # 최근 20일 평균 거래량 대비 당일 거래량이 3.0배 이상
            avg_vol_20 = sum(b['volume'] for b in prev_bars[-20:]) / 20
            is_volume_spike = vol >= avg_vol_20 * 2.5

            # 4. 조 (조정 기간 검증)
            # 돌파 직전 60일간 최고가와 최저가 진폭이 수축 상태였는지 체크
            prev_high_60 = max(b['high'] for b in prev_bars[-60:])
            prev_low_60 = min(b['low'] for b in prev_bars[-60:])
            if prev_low_60 > 0:
                is_consolidated = (prev_high_60 - prev_low_60) / prev_low_60 <= 0.35
            else:
