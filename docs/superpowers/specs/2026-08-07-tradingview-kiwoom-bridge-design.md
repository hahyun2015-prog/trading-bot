...
- **signal_source 전용 실시간 전환 명령 신규 개발**: `signal_source` 설정 변경은 ERA
  재시작 또는 익일 장준비(`_on_morning_prep_finished`) 시점에 반영된다(4.1절 운영 절차
  참고 — 당일 반영은 기존 `!선물시작` 재시작 명령 재사용으로 충분히 가능하므로, 이를 위한
  전용 신규 텔레그램 명령은 이번 스코프에 넣지 않는다. 필요해지면 별도 트랙).
...
사이클마다 재로드되는 게 아니다. 컴퓨터/ERA 프로세스를 계속 켜둔 채로 전환하려면 두 가지
경로가 있고, 둘 다 이번 구현에서 별도 코드 없이 그대로 성립한다.

1. **익일 자동 반영 (기본, 재시작 불필요)**: 장중이든 장마감 후든 아무 때나
   `config/signal_source.json` 값만 고쳐두면 된다. ERA를 껐다 켤 필요 없이 다음 거래일
   장전준비 때 자동으로 새 값을 읽어 그날부터 적용된다.
2. **당일 즉시 반영 (ERA 재시작)**: 오늘 장중에 바로 전환하려면 ERA 프로세스를
   재시작해야 한다. 기존 TCA 텔레그램 명령 `!선물시작`/`!시스템시작`
   (`tca_controller.py:930-948`)이 `_kill_era_process()`로 기존 프로세스를 정리하고
   재기동하는 경로를 이미 제공하므로, 이걸 그대로 쓰면 된다. 포지션/청산상태는
   `load_persisted_positions()`/`load_futures_exit_state()`로 복원되므로 재시작 자체는
   안전하게 설계돼 있다. 다만 재시작 도중(최대 20초) ERA가 신호를 받지 못하는 공백이
   생긴다는 점은 감안해야 한다.
...
혼동 방지를 위해 기록해두되, 이번 작업에서 고치지는 않는다.
...
era_order_manager.py
  ...
  - 하트비트 워치독(신규, 30초 주기 QTimer — 기존 `_futures_eod_fast_sync_tick`과 동일 패턴):
    신호소스 TRADINGVIEW + 포지션 보유 + 하트비트 15분 이상 유실 → 6절 fallback 발동
```
...
지지 않았는지"를 나중에 구분할 수 없어 9절 마지막 리스크(오설정 방치)를 조기에 발견하기
어려워지기 때문이다.
