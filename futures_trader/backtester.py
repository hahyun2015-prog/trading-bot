...
# (아래 sys.path 추가는 import 경로 확보용일 뿐, DB 경로는 기존대로 cwd 상대다 — 발견 N5)
...
def run_futures_backtest():
    ...
    # 다이버전스 사전 계산을 위해 RSI의 로컬 고점/저점 추적 (단순화된 윈도우 5봉 기준)
        ...
        for i in range(10, len(df)):
            ...
            if position == 0 and not force_close:
                ...
                if run_mode == 'DayOnly':
                    ...
                    # 야간장만 허용 (18:00 ~ 04:50) -> 오전 6시부터 오후 5시 59분까지 진입 차단
                    if 6 <= current_time.hour < 18:
                        can_enter = False
                
                if can_enter:
                    # Strict Mode: 밴드 정확히 터치 & RSI 30/70
